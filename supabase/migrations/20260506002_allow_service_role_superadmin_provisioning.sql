...
BEGIN
  ...
  v_provisioning_source := NULLIF(BTRIM(COALESCE(NEW.raw_app_meta_data->>'provisioning_source', '')), '');
  v_request_role := NULLIF(BTRIM(COALESCE(auth.role(), '')), '');
  ...
  IF v_provisioning_source IS DISTINCT FROM 'superadmin' AND v_request_role IS DISTINCT FROM 'service_role' THEN
    RAISE EXCEPTION 'PUBLIC_SIGNUP_DISABLED: tenant creation is restricted to platform administrators';
  END IF;
