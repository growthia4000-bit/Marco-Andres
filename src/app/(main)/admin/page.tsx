import { redirect } from 'next/navigation'
import { createClient } from '@/lib/supabase/server'
import { createClient as createSupabaseClient } from '@supabase/supabase-js'
import { getRequestLocale } from '@/i18n/server'
import { messages } from '@/i18n/messages'
import SuperadminDashboard from './SuperadminDashboard'
import { reactivateTenantAction, suspendTenantAction, updateCommercialStatusAction, createTenantAction } from './actions'

type TenantRow = {
  id: string
  name: string
  slug: string
  status: 'active' | 'inactive' | 'suspended'
  email: string | null
  trial_ends_at: string | null
  suspended_at: string | null
  created_at: string
}

type SubscriptionRow = {
  tenant_id: string
  status: 'trial' | 'active' | 'past_due' | 'canceled' | 'suspended'
  billing_cycle: string | null
  current_period_end: string | null
  trial_ends_at: string | null
  plans: {
    slug: string
    name: string
    max_users: number
  }[] | null
}

type CapacitySnapshot = {
  tenant_id: string
  tenant_status: 'active' | 'inactive' | 'suspended'
  subscription_status: 'trial' | 'active' | 'past_due' | 'canceled' | 'suspended' | null
  plan_slug: string | null
  billable_user_count: number
  max_users: number | null
  can_create_more_users: boolean
  is_operational: boolean
}

async function getCapacitySnapshot(supabase: Awaited<ReturnType<typeof createClient>>, tenantId: string) {
  const { data, error } = await supabase.rpc('app_tenant_capacity_snapshot', {
    target_tenant_id: tenantId,
  })

  if (error) {
    return null
  }

  return (data?.[0] ?? null) as CapacitySnapshot | null
}

export default async function AdminPage() {
  const supabase = await createClient()
  const adminSupabase = createSupabaseClient(
    process.env.NEXT_PUBLIC_SUPABASE_URL!,
    process.env.SUPABASE_SERVICE_ROLE_KEY!
  )
  const locale = await getRequestLocale()
  const dict = messages[locale]

  const {
    data: { user },
  } = await supabase.auth.getUser()

  if (!user) {
    redirect('/login')
  }

  const { data: currentUser } = await adminSupabase
    .from('users')
    .select('tenant_id, global_role, full_name, email')
    .eq('id', user.id)
    .single()

  if (!currentUser || currentUser.global_role !== 'superadmin') {
    redirect('/dashboard')
  }

  const tenantSlug = currentUser.tenant_id
    ? (await adminSupabase
        .from('tenants')
        .select('slug')
        .eq('id', currentUser.tenant_id)
        .maybeSingle()).data?.slug ?? null
    : null

  const [{ data: tenantsData, error: tenantsError }, { data: subscriptionsData, error: subscriptionsError }, { data: plansData, error: plansError }] = await Promise.all([
    adminSupabase
      .from('tenants')
      .select('id, name, slug, status, email, trial_ends_at, suspended_at, created_at')
      .order('created_at', { ascending: false }),
    adminSupabase
      .from('subscriptions')
      .select('tenant_id, status, billing_cycle, current_period_end, trial_ends_at, plans(slug, name, max_users)')
      .eq('is_current', true),
    adminSupabase
      .from('plans')
      .select('id, slug, name, max_users')
      .order('monthly_price_cents', { ascending: true }),
  ])

  if (tenantsError || subscriptionsError || plansError) {
    return (
      <div className="min-h-screen bg-slate-50">
        <header className="bg-white border-b border-slate-200 px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-slate-500">{String(dict.admin.superadmin)}</p>
              <h1 className="text-2xl font-semibold text-slate-900">{String(dict.admin.globalPanel)}</h1>
            </div>
          </div>
        </header>
        <main className="p-6">
          <div className="bg-white rounded-xl border border-red-200 p-6 text-red-600">
            {String(dict.admin.loadError)}
          </div>
        </main>
      </div>
    )
  }

  const tenants = (tenantsData ?? []) as TenantRow[]
  const plans = plansData ?? []
  const subscriptions = ((subscriptionsData ?? []) as SubscriptionRow[]).map((item) => ({
    ...item,
    plans: item.plans?.[0] ? [item.plans[0]] : null,
  }))
  const subscriptionByTenant = new Map(
    subscriptions.map((item) => [
      item.tenant_id,
      {
        ...item,
        plans: item.plans?.[0] ?? null,
      },
    ])
  )

  const capacityEntries = await Promise.all(
    tenants.map(async (tenant) => [tenant.id, await getCapacitySnapshot(adminSupabase, tenant.id)] as const)
  )

  const capacityByTenant = new Map(capacityEntries)

  return (
    <SuperadminDashboard
      currentUser={{
        fullName: currentUser.full_name ?? currentUser.email ?? 'Superadmin',
        email: currentUser.email ?? user.email ?? '',
        tenantSlug,
      }}
      tenants={tenants.map((tenant) => ({
        ...tenant,
        subscription: subscriptionByTenant.get(tenant.id) ?? null,
        capacity: capacityByTenant.get(tenant.id) ?? null,
      }))}
      plans={plans}
      suspendTenantAction={suspendTenantAction}
      reactivateTenantAction={reactivateTenantAction}
      updateCommercialStatusAction={updateCommercialStatusAction}
      createTenantAction={createTenantAction}
    />
  )
}
